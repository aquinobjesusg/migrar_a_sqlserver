...
@lang('dicc.Pbcatedt')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfPbe_name')</th><th>@lang('dicc.NameOfPbe_edit')</th><th>@lang('dicc.NameOfPbe_type')</th><th>@lang('dicc.NameOfPbe_cntr')</th><th>@lang('dicc.NameOfPbe_seqn')</th><th>@lang('dicc.NameOfPbe_flag')</th><th>@lang('dicc.NameOfPbe_work')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->pbe_name }}</td><td>{{ $registro->pbe_edit }}</td><td>{{ $registro->pbe_type }}</td><td>{{ $registro->pbe_cntr }}</td><td>{{ $registro->pbe_seqn }}</td><td>{{ $registro->pbe_flag }}</td><td>{{ $registro->pbe_work }}</td>
                    <td>
                    @if( $btn_ver == 1 )
