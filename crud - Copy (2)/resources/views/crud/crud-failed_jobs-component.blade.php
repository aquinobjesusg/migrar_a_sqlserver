...
@lang('dicc.Failed_jobs')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfUuid')</th><th>@lang('dicc.NameOfConnection')</th><th>@lang('dicc.NameOfQueue')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->uuid }}</td><td>{{ $registro->connection }}</td><td>{{ $registro->queue }}</td>
                    <td>
                    @if( $btn_ver == 1 )
