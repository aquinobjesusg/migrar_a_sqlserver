...
@lang('dicc.Motivo_factura')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCodigo')</th><th>@lang('dicc.NameOfDescripcion')</th><th>@lang('dicc.NameOfMonto')</th><th>@lang('dicc.NameOfMonto_seg')</th><th>@lang('dicc.NameOfTipo')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->codigo }}</td><td>{{ $registro->descripcion }}</td><td>{{ $registro->monto }}</td><td>{{ $registro->monto_seg }}</td><td>{{ $registro->tipo }}</td>
                    <td>
                    @if( $btn_ver == 1 )
