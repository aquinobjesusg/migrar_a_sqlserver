...
@lang('dicc.Pbcatfmt')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfPbf_name')</th><th>@lang('dicc.NameOfPbf_frmt')</th><th>@lang('dicc.NameOfPbf_type')</th><th>@lang('dicc.NameOfPbf_cntr')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->pbf_name }}</td><td>{{ $registro->pbf_frmt }}</td><td>{{ $registro->pbf_type }}</td><td>{{ $registro->pbf_cntr }}</td>
                    <td>
                    @if( $btn_ver == 1 )
