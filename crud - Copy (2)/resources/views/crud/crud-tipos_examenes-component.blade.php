...
@lang('dicc.Tipos_examenes')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCodetipo')</th><th>@lang('dicc.NameOfTipo')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->codetipo }}</td><td>{{ $registro->tipo }}</td>
                    <td>
                    @if( $btn_ver == 1 )
