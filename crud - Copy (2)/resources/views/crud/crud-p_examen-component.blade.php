...
@lang('dicc.P_examen')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCodexa')</th><th>@lang('dicc.NameOfCodetip')</th><th>@lang('dicc.NameOfExamen')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->codexa }}</td><td>{{ $registro->codetip }}</td><td>{{ $registro->examen }}</td>
                    <td>
                    @if( $btn_ver == 1 )
