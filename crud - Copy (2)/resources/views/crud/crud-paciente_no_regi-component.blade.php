...
@lang('dicc.Paciente_no_regi')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfRegistro')</th><th>@lang('dicc.NameOfApellidos')</th><th>@lang('dicc.NameOfNombres')</th><th>@lang('dicc.NameOfCelular')</th><th>@lang('dicc.NameOfMotivo')</th><th>@lang('dicc.NameOfFecha')</th><th>@lang('dicc.NameOfMedico')</th><th>@lang('dicc.NameOfRegistrado')</th><th>@lang('dicc.NameOfHora')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->registro }}</td><td>{{ $registro->apellidos }}</td><td>{{ $registro->nombres }}</td><td>{{ $registro->celular }}</td><td>{{ $registro->motivo }}</td><td>{{ $registro->fecha }}</td><td>{{ $registro->medico }}</td><td>{{ $registro->registrado }}</td><td>{{ $registro->hora }}</td>
                    <td>
                    @if( $btn_ver == 1 )
