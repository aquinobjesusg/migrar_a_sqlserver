...
@lang('dicc.Dias_semana')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfFecha')</th><th>@lang('dicc.NameOfDia_semana')</th><th>@lang('dicc.NameOfSemana')</th><th>@lang('dicc.NameOfAno')</th><th>@lang('dicc.NameOfDescripcion')</th><th>@lang('dicc.NameOfTipo_dia')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->fecha }}</td><td>{{ $registro->dia_semana }}</td><td>{{ $registro->semana }}</td><td>{{ $registro->ano }}</td><td>{{ $registro->descripcion }}</td><td>{{ $registro->tipo_dia }}</td>
                    <td>
                    @if( $btn_ver == 1 )
