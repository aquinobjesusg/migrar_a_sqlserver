...
@lang('dicc.Antecedentes')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCodantecedente')</th><th>@lang('dicc.NameOfDescripcion')</th><th>@lang('dicc.NameOfCodtipo')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->codantecedente }}</td><td>{{ $registro->descripcion }}</td><td>{{ $registro->codtipo }}</td>
                    <td>
                    @if( $btn_ver == 1 )
