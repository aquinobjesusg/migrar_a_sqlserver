...
@lang('dicc.Representante')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNumhistoria')</th><th>@lang('dicc.NameOfNombre')</th><th>@lang('dicc.NameOfCodeparentesco')</th><th>@lang('dicc.NameOfDireccion')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->numhistoria }}</td><td>{{ $registro->nombre }}</td><td>{{ $registro->codeparentesco }}</td><td>{{ $registro->direccion }}</td>
                    <td>
                    @if( $btn_ver == 1 )
