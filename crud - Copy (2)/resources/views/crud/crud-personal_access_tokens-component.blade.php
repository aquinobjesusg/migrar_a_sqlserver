...
@lang('dicc.Personal_access_tokens')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfTokenable_type')</th><th>@lang('dicc.NameOfTokenable_id')</th><th>@lang('dicc.NameOfName')</th><th>@lang('dicc.NameOfToken')</th><th>@lang('dicc.NameOfAbilities')</th><th>@lang('dicc.NameOfLast_used_at')</th><th>@lang('dicc.NameOfExpires_at')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->tokenable_type }}</td><td>{{ $registro->tokenable_id }}</td><td>{{ $registro->name }}</td><td>{{ $registro->token }}</td><td>{{ $registro->abilities }}</td><td>{{ $registro->last_used_at }}</td><td>{{ $registro->expires_at }}</td>
                    <td>
                    @if( $btn_ver == 1 )
