...
@lang('dicc.Listado')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfTipo')</th><th>@lang('dicc.NameOfDescripcion_tipo')</th><th>@lang('dicc.NameOfSegundo')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->tipo }}</td><td>{{ $registro->descripcion_tipo }}</td><td>{{ $registro->segundo }}</td>
                    <td>
                    @if( $btn_ver == 1 )
