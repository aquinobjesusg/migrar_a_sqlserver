...
@lang('dicc.P_epecial')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCodespe')</th><th>@lang('dicc.NameOfEspeciali')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->codespe }}</td><td>{{ $registro->especiali }}</td>
                    <td>
                    @if( $btn_ver == 1 )
