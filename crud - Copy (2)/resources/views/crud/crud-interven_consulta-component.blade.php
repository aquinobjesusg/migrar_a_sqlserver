...
@lang('dicc.Interven_consulta')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfHistoria')</th><th>@lang('dicc.NameOfConsulta')</th><th>@lang('dicc.NameOfCodigo_inter')</th><th>@lang('dicc.NameOfDetalle')</th><th>@lang('dicc.NameOfDescripcion')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->historia }}</td><td>{{ $registro->consulta }}</td><td>{{ $registro->codigo_inter }}</td><td>{{ $registro->detalle }}</td><td>{{ $registro->descripcion }}</td>
                    <td>
                    @if( $btn_ver == 1 )
