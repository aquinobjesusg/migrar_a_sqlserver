...
@lang('dicc.Presupuesto_planti')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfConsecutivo')</th><th>@lang('dicc.NameOfTipo_precio')</th><th>@lang('dicc.NameOfTotal_costo')</th><th>@lang('dicc.NameOfTotal_final')</th><th>@lang('dicc.NameOfNotas')</th><th>@lang('dicc.NameOfTipo_doc')</th><th>@lang('dicc.NameOfNom_presupuesto')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->consecutivo }}</td><td>{{ $registro->tipo_precio }}</td><td>{{ $registro->total_costo }}</td><td>{{ $registro->total_final }}</td><td>{{ $registro->notas }}</td><td>{{ $registro->tipo_doc }}</td><td>{{ $registro->nom_presupuesto }}</td>
                    <td>
                    @if( $btn_ver == 1 )
