...
@lang('dicc.Users')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfName')</th><th>@lang('dicc.NameOfEmail')</th><th>@lang('dicc.NameOfEmail_verified_at')</th><th>@lang('dicc.NameOfPassword')</th><th>@lang('dicc.NameOfTwo_factor_secret')</th><th>@lang('dicc.NameOfTwo_factor_recovery_codes')</th><th>@lang('dicc.NameOfRemember_token')</th><th>@lang('dicc.NameOfAvatar')</th><th>@lang('dicc.NameOfRole')</th><th>@lang('dicc.NameOfIn_medico')</th><th>@lang('dicc.NameOfIn_paciente')</th><th>@lang('dicc.NameOfActivo')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->name }}</td><td>{{ $registro->email }}</td><td>{{ $registro->email_verified_at }}</td><td>{{ $registro->password }}</td><td>{{ $registro->two_factor_secret }}</td><td>{{ $registro->two_factor_recovery_codes }}</td><td>{{ $registro->remember_token }}</td><td>{{ $registro->avatar }}</td><td>{{ $registro->role }}</td><td>{{ $registro->in_medico }}</td><td>{{ $registro->in_paciente }}</td><td>{{ $registro->activo }}</td>
                    <td>
                    @if( $btn_ver == 1 )
