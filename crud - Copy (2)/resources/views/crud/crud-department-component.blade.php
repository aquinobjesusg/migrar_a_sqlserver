...
@lang('dicc.Department')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfDept_id')</th><th>@lang('dicc.NameOfDept_name')</th><th>@lang('dicc.NameOfDept_head_id')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->dept_id }}</td><td>{{ $registro->dept_name }}</td><td>{{ $registro->dept_head_id }}</td>
                    <td>
                    @if( $btn_ver == 1 )
