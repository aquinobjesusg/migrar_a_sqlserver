...
@lang('dicc.Pago_quiru')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCod_pagos')</th><th>@lang('dicc.NameOfConsecuti')</th><th>@lang('dicc.NameOfFecha')</th><th>@lang('dicc.NameOfMonto_total')</th><th>@lang('dicc.NameOfAbono')</th><th>@lang('dicc.NameOfResta')</th><th>@lang('dicc.NameOfPago')</th><th>@lang('dicc.NameOfHora_pago')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->cod_pagos }}</td><td>{{ $registro->consecuti }}</td><td>{{ $registro->fecha }}</td><td>{{ $registro->monto_total }}</td><td>{{ $registro->abono }}</td><td>{{ $registro->resta }}</td><td>{{ $registro->pago }}</td><td>{{ $registro->hora_pago }}</td>
                    <td>
                    @if( $btn_ver == 1 )
