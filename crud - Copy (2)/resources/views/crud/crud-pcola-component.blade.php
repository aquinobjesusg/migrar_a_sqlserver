...
@lang('dicc.Pcola')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfAtendido')</th><th>@lang('dicc.NameOfConse')</th><th>@lang('dicc.NameOfEstado')</th><th>@lang('dicc.NameOfFecha')</th><th>@lang('dicc.NameOfHora_fin')</th><th>@lang('dicc.NameOfHora_ini')</th><th>@lang('dicc.NameOfMedico')</th><th>@lang('dicc.NameOfMotivo')</th><th>@lang('dicc.NameOfNhistoria')</th><th>@lang('dicc.NameOfNumorden')</th><th>@lang('dicc.NameOfTipo')</th><th>@lang('dicc.NameOfTurno')</th><th>@lang('dicc.NameOfFechac')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->atendido }}</td><td>{{ $registro->conse }}</td><td>{{ $registro->estado }}</td><td>{{ $registro->fecha }}</td><td>{{ $registro->hora_fin }}</td><td>{{ $registro->hora_ini }}</td><td>{{ $registro->medico }}</td><td>{{ $registro->motivo }}</td><td>{{ $registro->nhistoria }}</td><td>{{ $registro->numorden }}</td><td>{{ $registro->tipo }}</td><td>{{ $registro->turno }}</td><td>{{ $registro->fechac }}</td>
                    <td>
                    @if( $btn_ver == 1 )
