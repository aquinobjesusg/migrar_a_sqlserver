...
@lang('dicc.Clinicas')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCod_clin')</th><th>@lang('dicc.NameOfNom_clin')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->cod_clin }}</td><td>{{ $registro->nom_clin }}</td>
                    <td>
                    @if( $btn_ver == 1 )
