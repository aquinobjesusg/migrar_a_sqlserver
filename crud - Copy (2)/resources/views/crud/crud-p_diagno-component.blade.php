...
@lang('dicc.P_diagno')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCodiagn')</th><th>@lang('dicc.NameOfDescrip')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->codiagn }}</td><td>{{ $registro->descrip }}</td>
                    <td>
                    @if( $btn_ver == 1 )
