...
@lang('dicc.Hospitalizacion')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNrohistoria')</th><th>@lang('dicc.NameOfNroconsulta')</th><th>@lang('dicc.NameOfDiagprev')</th><th>@lang('dicc.NameOfIndiprev')</th><th>@lang('dicc.NameOfDieta')</th><th>@lang('dicc.NameOfHabitacion')</th><th>@lang('dicc.NameOfHospital')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nrohistoria }}</td><td>{{ $registro->nroconsulta }}</td><td>{{ $registro->diagprev }}</td><td>{{ $registro->indiprev }}</td><td>{{ $registro->dieta }}</td><td>{{ $registro->habitacion }}</td><td>{{ $registro->hospital }}</td>
                    <td>
                    @if( $btn_ver == 1 )
