...
@lang('dicc.Examenes')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCodeexamen')</th><th>@lang('dicc.NameOfExamen')</th><th>@lang('dicc.NameOfCodetipo')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->codeexamen }}</td><td>{{ $registro->examen }}</td><td>{{ $registro->codetipo }}</td>
                    <td>
                    @if( $btn_ver == 1 )
