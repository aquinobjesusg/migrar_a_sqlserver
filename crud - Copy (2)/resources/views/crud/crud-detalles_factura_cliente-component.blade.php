...
@lang('dicc.Detalles_factura_cliente')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCod_inventario')</th><th>@lang('dicc.NameOfNumfactura')</th><th>@lang('dicc.NameOfTipo_articulo')</th><th>@lang('dicc.NameOfNombre')</th><th>@lang('dicc.NameOfPrecio')</th><th>@lang('dicc.NameOfDescuento')</th><th>@lang('dicc.NameOfCantidad')</th><th>@lang('dicc.NameOfFecha_doc')</th><th>@lang('dicc.NameOfTipo_precio')</th><th>@lang('dicc.NameOfTotal_articulo')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->cod_inventario }}</td><td>{{ $registro->numfactura }}</td><td>{{ $registro->tipo_articulo }}</td><td>{{ $registro->nombre }}</td><td>{{ $registro->precio }}</td><td>{{ $registro->descuento }}</td><td>{{ $registro->cantidad }}</td><td>{{ $registro->fecha_doc }}</td><td>{{ $registro->tipo_precio }}</td><td>{{ $registro->total_articulo }}</td>
                    <td>
                    @if( $btn_ver == 1 )
