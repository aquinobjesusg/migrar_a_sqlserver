...
@lang('dicc.Imagen_pacientes')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNrohistoria')</th><th>@lang('dicc.NameOfImagen')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nrohistoria }}</td><td>{{ $registro->imagen }}</td>
                    <td>
                    @if( $btn_ver == 1 )
