...
@lang('dicc.Pvademe')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCodmedi')</th><th>@lang('dicc.NameOfNombrege')</th><th>@lang('dicc.NameOfNombreco')</th><th>@lang('dicc.NameOfDosifi')</th><th>@lang('dicc.NameOfUso')</th><th>@lang('dicc.NameOfPresenta')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->codmedi }}</td><td>{{ $registro->nombrege }}</td><td>{{ $registro->nombreco }}</td><td>{{ $registro->dosifi }}</td><td>{{ $registro->uso }}</td><td>{{ $registro->presenta }}</td>
                    <td>
                    @if( $btn_ver == 1 )
