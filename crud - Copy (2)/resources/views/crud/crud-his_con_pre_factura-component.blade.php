...
@lang('dicc.His_con_pre_factura')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfId_registro')</th><th>@lang('dicc.NameOfNumhistoria')</th><th>@lang('dicc.NameOfNroconsulta')</th><th>@lang('dicc.NameOfCodigo')</th><th>@lang('dicc.NameOfDescripcion')</th><th>@lang('dicc.NameOfCantidad')</th><th>@lang('dicc.NameOfMonto')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->id_registro }}</td><td>{{ $registro->numhistoria }}</td><td>{{ $registro->nroconsulta }}</td><td>{{ $registro->codigo }}</td><td>{{ $registro->descripcion }}</td><td>{{ $registro->cantidad }}</td><td>{{ $registro->monto }}</td>
                    <td>
                    @if( $btn_ver == 1 )
