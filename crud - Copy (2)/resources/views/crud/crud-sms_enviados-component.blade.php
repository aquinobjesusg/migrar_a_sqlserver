...
@lang('dicc.Sms_enviados')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfConta')</th><th>@lang('dicc.NameOfUsuario')</th><th>@lang('dicc.NameOfMedico')</th><th>@lang('dicc.NameOfProveedor')</th><th>@lang('dicc.NameOfNumero')</th><th>@lang('dicc.NameOfMensaje')</th><th>@lang('dicc.NameOfFecha')</th><th>@lang('dicc.NameOfTipo')</th><th>@lang('dicc.NameOfHistoria')</th><th>@lang('dicc.NameOfConsulta')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->conta }}</td><td>{{ $registro->usuario }}</td><td>{{ $registro->medico }}</td><td>{{ $registro->proveedor }}</td><td>{{ $registro->numero }}</td><td>{{ $registro->mensaje }}</td><td>{{ $registro->fecha }}</td><td>{{ $registro->tipo }}</td><td>{{ $registro->historia }}</td><td>{{ $registro->consulta }}</td>
                    <td>
                    @if( $btn_ver == 1 )
