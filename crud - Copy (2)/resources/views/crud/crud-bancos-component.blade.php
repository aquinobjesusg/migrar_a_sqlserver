...
@lang('dicc.Bancos')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCod_banco')</th><th>@lang('dicc.NameOfNombre_banco')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->cod_banco }}</td><td>{{ $registro->nombre_banco }}</td>
                    <td>
                    @if( $btn_ver == 1 )
