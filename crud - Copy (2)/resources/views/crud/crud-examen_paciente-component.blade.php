...
@lang('dicc.Examen_paciente')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNrohistoria')</th><th>@lang('dicc.NameOfNroconsulta')</th><th>@lang('dicc.NameOfCodeexamen')</th><th>@lang('dicc.NameOfResultado')</th><th>@lang('dicc.NameOfVinculante')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nrohistoria }}</td><td>{{ $registro->nroconsulta }}</td><td>{{ $registro->codeexamen }}</td><td>{{ $registro->resultado }}</td><td>{{ $registro->vinculante }}</td>
                    <td>
                    @if( $btn_ver == 1 )
