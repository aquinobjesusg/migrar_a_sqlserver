...
@lang('dicc.Pdiagno')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNhisto')</th><th>@lang('dicc.NameOfNconsul')</th><th>@lang('dicc.NameOfCodedia')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nhisto }}</td><td>{{ $registro->nconsul }}</td><td>{{ $registro->codedia }}</td>
                    <td>
                    @if( $btn_ver == 1 )
