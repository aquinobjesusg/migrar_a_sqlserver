...
@lang('dicc.Sms_compra')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfConse_compra')</th><th>@lang('dicc.NameOfFecha_compra')</th><th>@lang('dicc.NameOfMonto_compra')</th><th>@lang('dicc.NameOfCantidad_compra')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->conse_compra }}</td><td>{{ $registro->fecha_compra }}</td><td>{{ $registro->monto_compra }}</td><td>{{ $registro->cantidad_compra }}</td>
                    <td>
                    @if( $btn_ver == 1 )
