...
@lang('dicc.P_seg')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCodeseg')</th><th>@lang('dicc.NameOfNombre')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->codeseg }}</td><td>{{ $registro->nombre }}</td>
                    <td>
                    @if( $btn_ver == 1 )
