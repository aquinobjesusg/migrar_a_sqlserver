...
@lang('dicc.Reg_empl_tipo_nomina')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfTipo_nomina')</th><th>@lang('dicc.NameOfNombre_nomina')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->tipo_nomina }}</td><td>{{ $registro->nombre_nomina }}</td>
                    <td>
                    @if( $btn_ver == 1 )
