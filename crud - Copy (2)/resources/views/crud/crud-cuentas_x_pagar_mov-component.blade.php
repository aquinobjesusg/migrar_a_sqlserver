...
@lang('dicc.Cuentas_x_pagar_mov')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNro_registro')</th><th>@lang('dicc.NameOfNro_cxp')</th><th>@lang('dicc.NameOfNro_mov')</th><th>@lang('dicc.NameOfFecha_emision')</th><th>@lang('dicc.NameOfNro_documento')</th><th>@lang('dicc.NameOfTip_documento')</th><th>@lang('dicc.NameOfMonto_pagar')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nro_registro }}</td><td>{{ $registro->nro_cxp }}</td><td>{{ $registro->nro_mov }}</td><td>{{ $registro->fecha_emision }}</td><td>{{ $registro->nro_documento }}</td><td>{{ $registro->tip_documento }}</td><td>{{ $registro->monto_pagar }}</td>
                    <td>
                    @if( $btn_ver == 1 )
