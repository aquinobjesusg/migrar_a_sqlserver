...
@lang('dicc.Gsistema')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfId_medico')</th><th>@lang('dicc.NameOfNomcorto')</th><th>@lang('dicc.NameOfClave')</th><th>@lang('dicc.NameOfUlthisto')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->id_medico }}</td><td>{{ $registro->nomcorto }}</td><td>{{ $registro->clave }}</td><td>{{ $registro->ulthisto }}</td>
                    <td>
                    @if( $btn_ver == 1 )
