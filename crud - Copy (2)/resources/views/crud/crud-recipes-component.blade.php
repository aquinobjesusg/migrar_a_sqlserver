...
@lang('dicc.Recipes')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNrohistoria')</th><th>@lang('dicc.NameOfNroconsulta')</th><th>@lang('dicc.NameOfCodemedicina')</th><th>@lang('dicc.NameOfIndicaciones')</th><th>@lang('dicc.NameOfCantidad')</th><th>@lang('dicc.NameOfOrden')</th><th>@lang('dicc.NameOfDescripcion')</th><th>@lang('dicc.NameOfFecha')</th><th>@lang('dicc.NameOfRecipe')</th><th>@lang('dicc.NameOfComple')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nrohistoria }}</td><td>{{ $registro->nroconsulta }}</td><td>{{ $registro->codemedicina }}</td><td>{{ $registro->indicaciones }}</td><td>{{ $registro->cantidad }}</td><td>{{ $registro->orden }}</td><td>{{ $registro->descripcion }}</td><td>{{ $registro->fecha }}</td><td>{{ $registro->recipe }}</td><td>{{ $registro->comple }}</td>
                    <td>
                    @if( $btn_ver == 1 )
