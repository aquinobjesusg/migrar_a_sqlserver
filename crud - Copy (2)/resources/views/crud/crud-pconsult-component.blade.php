...
@lang('dicc.Pconsult')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNconsul')</th><th>@lang('dicc.NameOfNhisto')</th><th>@lang('dicc.NameOfEnfactual')</th><th>@lang('dicc.NameOfEvolu')</th><th>@lang('dicc.NameOfTratami')</th><th>@lang('dicc.NameOfExa_fi')</th><th>@lang('dicc.NameOfImage')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nconsul }}</td><td>{{ $registro->nhisto }}</td><td>{{ $registro->enfactual }}</td><td>{{ $registro->evolu }}</td><td>{{ $registro->tratami }}</td><td>{{ $registro->exa_fi }}</td><td>{{ $registro->image }}</td>
                    <td>
                    @if( $btn_ver == 1 )
