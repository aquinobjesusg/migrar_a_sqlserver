...
@lang('dicc.P_ante')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCodante')</th><th>@lang('dicc.NameOfCodtipo')</th><th>@lang('dicc.NameOfDescrip')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->codante }}</td><td>{{ $registro->codtipo }}</td><td>{{ $registro->descrip }}</td>
                    <td>
                    @if( $btn_ver == 1 )
