...
@lang('dicc.Motivo_factura_prov')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfId_fac_prov')</th><th>@lang('dicc.NameOfDes_concepto')</th><th>@lang('dicc.NameOfMonto')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->id_fac_prov }}</td><td>{{ $registro->des_concepto }}</td><td>{{ $registro->monto }}</td>
                    <td>
                    @if( $btn_ver == 1 )
