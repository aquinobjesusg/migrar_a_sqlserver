...
@lang('dicc.Emision_pagos_detalle')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNro_registro')</th><th>@lang('dicc.NameOfNro_emision')</th><th>@lang('dicc.NameOfNro_cxp')</th><th>@lang('dicc.NameOfNro_mov')</th><th>@lang('dicc.NameOfOrigen_cxp')</th><th>@lang('dicc.NameOfCxp_codigo')</th><th>@lang('dicc.NameOfCxp_descripcion')</th><th>@lang('dicc.NameOfFecha_emision')</th><th>@lang('dicc.NameOfNro_documento')</th><th>@lang('dicc.NameOfTip_documento')</th><th>@lang('dicc.NameOfSaldo_pagar')</th><th>@lang('dicc.NameOfMonto_pagar')</th><th>@lang('dicc.NameOfEstado')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nro_registro }}</td><td>{{ $registro->nro_emision }}</td><td>{{ $registro->nro_cxp }}</td><td>{{ $registro->nro_mov }}</td><td>{{ $registro->origen_cxp }}</td><td>{{ $registro->cxp_codigo }}</td><td>{{ $registro->cxp_descripcion }}</td><td>{{ $registro->fecha_emision }}</td><td>{{ $registro->nro_documento }}</td><td>{{ $registro->tip_documento }}</td><td>{{ $registro->saldo_pagar }}</td><td>{{ $registro->monto_pagar }}</td><td>{{ $registro->estado }}</td>
                    <td>
                    @if( $btn_ver == 1 )
