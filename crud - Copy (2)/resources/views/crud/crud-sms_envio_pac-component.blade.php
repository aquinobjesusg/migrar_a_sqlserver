...
@lang('dicc.Sms_envio_pac')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNrohistoria')</th><th>@lang('dicc.NameOfNroconsulta')</th><th>@lang('dicc.NameOfNumero_cel')</th><th>@lang('dicc.NameOfTexto_sms')</th><th>@lang('dicc.NameOfOrden')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nrohistoria }}</td><td>{{ $registro->nroconsulta }}</td><td>{{ $registro->numero_cel }}</td><td>{{ $registro->texto_sms }}</td><td>{{ $registro->orden }}</td>
                    <td>
                    @if( $btn_ver == 1 )
