...
@lang('dicc.Registro_empleados_eje')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNro_nomina')</th><th>@lang('dicc.NameOfDescripcion')</th><th>@lang('dicc.NameOfFecha_aplicacion')</th><th>@lang('dicc.NameOfStatus')</th><th>@lang('dicc.NameOfTipo_nomina')</th><th>@lang('dicc.NameOfFrecuencia_nomina')</th><th>@lang('dicc.NameOfMonto_total')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nro_nomina }}</td><td>{{ $registro->descripcion }}</td><td>{{ $registro->fecha_aplicacion }}</td><td>{{ $registro->status }}</td><td>{{ $registro->tipo_nomina }}</td><td>{{ $registro->frecuencia_nomina }}</td><td>{{ $registro->monto_total }}</td>
                    <td>
                    @if( $btn_ver == 1 )
