...
@lang('dicc.Intenven_servi')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCodigo')</th><th>@lang('dicc.NameOfNombre')</th><th>@lang('dicc.NameOfPrecio_principal')</th><th>@lang('dicc.NameOfPrecio_auxiliar')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->codigo }}</td><td>{{ $registro->nombre }}</td><td>{{ $registro->precio_principal }}</td><td>{{ $registro->precio_auxiliar }}</td>
                    <td>
                    @if( $btn_ver == 1 )
