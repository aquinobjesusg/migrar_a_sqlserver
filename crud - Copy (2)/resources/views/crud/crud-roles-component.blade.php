...
@lang('dicc.Roles')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfName')</th><th>@lang('dicc.NameOfActivo')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->name }}</td><td>{{ $registro->activo }}</td>
                    <td>
                    @if( $btn_ver == 1 )
