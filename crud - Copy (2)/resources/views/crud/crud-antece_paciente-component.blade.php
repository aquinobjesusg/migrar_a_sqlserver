...
@lang('dicc.Antece_paciente')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNumhistoria')</th><th>@lang('dicc.NameOfCodeantecedente')</th><th>@lang('dicc.NameOfDetalles')</th><th>@lang('dicc.NameOfDescripcion')</th><th>@lang('dicc.NameOfTipo')</th><th>@lang('dicc.NameOfTipo2')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->numhistoria }}</td><td>{{ $registro->codeantecedente }}</td><td>{{ $registro->detalles }}</td><td>{{ $registro->descripcion }}</td><td>{{ $registro->tipo }}</td><td>{{ $registro->tipo2 }}</td>
                    <td>
                    @if( $btn_ver == 1 )
