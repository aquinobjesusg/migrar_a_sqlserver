...
@lang('dicc.Doctores')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCedula')</th><th>@lang('dicc.NameOfApellidos')</th><th>@lang('dicc.NameOfNombres')</th><th>@lang('dicc.NameOfClinica')</th><th>@lang('dicc.NameOfDireccion')</th><th>@lang('dicc.NameOfTelefono')</th><th>@lang('dicc.NameOfCiudad')</th><th>@lang('dicc.NameOfNota')</th><th>@lang('dicc.NameOfCodeespecial')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->cedula }}</td><td>{{ $registro->apellidos }}</td><td>{{ $registro->nombres }}</td><td>{{ $registro->clinica }}</td><td>{{ $registro->direccion }}</td><td>{{ $registro->telefono }}</td><td>{{ $registro->ciudad }}</td><td>{{ $registro->nota }}</td><td>{{ $registro->codeespecial }}</td>
                    <td>
                    @if( $btn_ver == 1 )
