...
@lang('dicc.Reposo_paciente')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNrohistoria')</th><th>@lang('dicc.NameOfNroconsulta')</th><th>@lang('dicc.NameOfCodereposo')</th><th>@lang('dicc.NameOfFdesde')</th><th>@lang('dicc.NameOfNumdias')</th><th>@lang('dicc.NameOfObser_reposo')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nrohistoria }}</td><td>{{ $registro->nroconsulta }}</td><td>{{ $registro->codereposo }}</td><td>{{ $registro->fdesde }}</td><td>{{ $registro->numdias }}</td><td>{{ $registro->obser_reposo }}</td>
                    <td>
                    @if( $btn_ver == 1 )
