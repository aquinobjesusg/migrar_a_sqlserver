...
@lang('dicc.Tipos_documentos')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfTip_documento')</th><th>@lang('dicc.NameOfDes_documento')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->tip_documento }}</td><td>{{ $registro->des_documento }}</td>
                    <td>
                    @if( $btn_ver == 1 )
