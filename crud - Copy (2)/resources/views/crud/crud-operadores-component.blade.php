...
@lang('dicc.Operadores')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfPrimera')</th><th>@lang('dicc.NameOfSegunda')</th><th>@lang('dicc.NameOfNonbre')</th><th>@lang('dicc.NameOfNivel')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->primera }}</td><td>{{ $registro->segunda }}</td><td>{{ $registro->nonbre }}</td><td>{{ $registro->nivel }}</td>
                    <td>
                    @if( $btn_ver == 1 )
