...
@lang('dicc.Recipe_detalle')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNrohistoria')</th><th>@lang('dicc.NameOfNroconsulta')</th><th>@lang('dicc.NameOfRecipe')</th><th>@lang('dicc.NameOfFe_emision')</th><th>@lang('dicc.NameOfFe_vence')</th><th>@lang('dicc.NameOfNota')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nrohistoria }}</td><td>{{ $registro->nroconsulta }}</td><td>{{ $registro->recipe }}</td><td>{{ $registro->fe_emision }}</td><td>{{ $registro->fe_vence }}</td><td>{{ $registro->nota }}</td>
                    <td>
                    @if( $btn_ver == 1 )
