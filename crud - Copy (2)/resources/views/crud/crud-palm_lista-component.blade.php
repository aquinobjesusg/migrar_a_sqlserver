...
@lang('dicc.Palm_lista')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfId_codigo')</th><th>@lang('dicc.NameOfNombre_tabla')</th><th>@lang('dicc.NameOfOrdinal')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->id_codigo }}</td><td>{{ $registro->nombre_tabla }}</td><td>{{ $registro->ordinal }}</td>
                    <td>
                    @if( $btn_ver == 1 )
