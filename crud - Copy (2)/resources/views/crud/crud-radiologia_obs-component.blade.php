...
@lang('dicc.Radiologia_obs')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNumhistoria')</th><th>@lang('dicc.NameOfNumconsulta')</th><th>@lang('dicc.NameOfObservacion')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->numhistoria }}</td><td>{{ $registro->numconsulta }}</td><td>{{ $registro->observacion }}</td>
                    <td>
                    @if( $btn_ver == 1 )
