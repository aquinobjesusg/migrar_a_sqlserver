...
@lang('dicc.Emision_pagos')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNro_emision')</th><th>@lang('dicc.NameOfFecha_emision')</th><th>@lang('dicc.NameOfDescripcion')</th><th>@lang('dicc.NameOfCod_banco')</th><th>@lang('dicc.NameOfEstado')</th><th>@lang('dicc.NameOfMonto_pagar')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nro_emision }}</td><td>{{ $registro->fecha_emision }}</td><td>{{ $registro->descripcion }}</td><td>{{ $registro->cod_banco }}</td><td>{{ $registro->estado }}</td><td>{{ $registro->monto_pagar }}</td>
                    <td>
                    @if( $btn_ver == 1 )
