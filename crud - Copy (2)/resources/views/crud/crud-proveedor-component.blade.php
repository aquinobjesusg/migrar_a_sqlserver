...
@lang('dicc.Proveedor')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCod_prov')</th><th>@lang('dicc.NameOfProveedor')</th><th>@lang('dicc.NameOfRif')</th><th>@lang('dicc.NameOfDireccion')</th><th>@lang('dicc.NameOfTelefono')</th><th>@lang('dicc.NameOfContacto')</th><th>@lang('dicc.NameOfCelular')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->cod_prov }}</td><td>{{ $registro->proveedor }}</td><td>{{ $registro->rif }}</td><td>{{ $registro->direccion }}</td><td>{{ $registro->telefono }}</td><td>{{ $registro->contacto }}</td><td>{{ $registro->celular }}</td>
                    <td>
                    @if( $btn_ver == 1 )
