...
@lang('dicc.Especial')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCodeespecial')</th><th>@lang('dicc.NameOfEspecialidad')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->codeespecial }}</td><td>{{ $registro->especialidad }}</td>
                    <td>
                    @if( $btn_ver == 1 )
