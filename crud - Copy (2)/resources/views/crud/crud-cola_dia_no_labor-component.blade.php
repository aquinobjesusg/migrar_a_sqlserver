...
@lang('dicc.Cola_dia_no_labor')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfDia')</th><th>@lang('dicc.NameOfTipo')</th><th>@lang('dicc.NameOfMotivo')</th><th>@lang('dicc.NameOfMedico')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->dia }}</td><td>{{ $registro->tipo }}</td><td>{{ $registro->motivo }}</td><td>{{ $registro->medico }}</td>
                    <td>
                    @if( $btn_ver == 1 )
