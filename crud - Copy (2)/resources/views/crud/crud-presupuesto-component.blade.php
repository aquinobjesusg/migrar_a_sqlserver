...
@lang('dicc.Presupuesto')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCedula')</th><th>@lang('dicc.NameOfNombre')</th><th>@lang('dicc.NameOfDireccion')</th><th>@lang('dicc.NameOfTelefono')</th><th>@lang('dicc.NameOfNropresupuesto')</th><th>@lang('dicc.NameOfFecha')</th><th>@lang('dicc.NameOfCliente')</th><th>@lang('dicc.NameOfNota')</th><th>@lang('dicc.NameOfNumhistoria')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->cedula }}</td><td>{{ $registro->nombre }}</td><td>{{ $registro->direccion }}</td><td>{{ $registro->telefono }}</td><td>{{ $registro->nropresupuesto }}</td><td>{{ $registro->fecha }}</td><td>{{ $registro->cliente }}</td><td>{{ $registro->nota }}</td><td>{{ $registro->numhistoria }}</td>
                    <td>
                    @if( $btn_ver == 1 )
