...
@lang('dicc.Registro_empleados_eje_detalle')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNro_nomina')</th><th>@lang('dicc.NameOfNro_empleado')</th><th>@lang('dicc.NameOfNro_cxp')</th><th>@lang('dicc.NameOfMonto_empleado')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nro_nomina }}</td><td>{{ $registro->nro_empleado }}</td><td>{{ $registro->nro_cxp }}</td><td>{{ $registro->monto_empleado }}</td>
                    <td>
                    @if( $btn_ver == 1 )
