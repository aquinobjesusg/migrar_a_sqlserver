...
@lang('dicc.Diagnosticos')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCodediagnostico')</th><th>@lang('dicc.NameOfDescripcion')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->codediagnostico }}</td><td>{{ $registro->descripcion }}</td>
                    <td>
                    @if( $btn_ver == 1 )
