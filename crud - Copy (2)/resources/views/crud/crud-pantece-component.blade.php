...
@lang('dicc.Pantece')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNhisto')</th><th>@lang('dicc.NameOfCodante')</th><th>@lang('dicc.NameOfDetalle')</th><th>@lang('dicc.NameOfTipo')</th><th>@lang('dicc.NameOfDescrip')</th><th>@lang('dicc.NameOfConsul')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nhisto }}</td><td>{{ $registro->codante }}</td><td>{{ $registro->detalle }}</td><td>{{ $registro->tipo }}</td><td>{{ $registro->descrip }}</td><td>{{ $registro->consul }}</td>
                    <td>
                    @if( $btn_ver == 1 )
