...
@lang('dicc.Pbcatvld')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfPbv_name')</th><th>@lang('dicc.NameOfPbv_vald')</th><th>@lang('dicc.NameOfPbv_type')</th><th>@lang('dicc.NameOfPbv_cntr')</th><th>@lang('dicc.NameOfPbv_msg')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->pbv_name }}</td><td>{{ $registro->pbv_vald }}</td><td>{{ $registro->pbv_type }}</td><td>{{ $registro->pbv_cntr }}</td><td>{{ $registro->pbv_msg }}</td>
                    <td>
                    @if( $btn_ver == 1 )
