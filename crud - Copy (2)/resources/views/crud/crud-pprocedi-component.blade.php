...
@lang('dicc.Pprocedi')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfNhisto')</th><th>@lang('dicc.NameOfNconsul')</th><th>@lang('dicc.NameOfProcedi1')</th><th>@lang('dicc.NameOfProcedi2')</th><th>@lang('dicc.NameOfProcedi3')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->nhisto }}</td><td>{{ $registro->nconsul }}</td><td>{{ $registro->procedi1 }}</td><td>{{ $registro->procedi2 }}</td><td>{{ $registro->procedi3 }}</td>
                    <td>
                    @if( $btn_ver == 1 )
