...
@lang('dicc.Ppacient')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCedula')</th><th>@lang('dicc.NameOfApellidos')</th><th>@lang('dicc.NameOfNombres')</th><th>@lang('dicc.NameOfSexo')</th><th>@lang('dicc.NameOfFnacimient')</th><th>@lang('dicc.NameOfTelefono')</th><th>@lang('dicc.NameOfCodesegemp')</th><th>@lang('dicc.NameOfNhistoria')</th><th>@lang('dicc.NameOfUltconsu')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->cedula }}</td><td>{{ $registro->apellidos }}</td><td>{{ $registro->nombres }}</td><td>{{ $registro->sexo }}</td><td>{{ $registro->fnacimient }}</td><td>{{ $registro->telefono }}</td><td>{{ $registro->codesegemp }}</td><td>{{ $registro->nhistoria }}</td><td>{{ $registro->ultconsu }}</td>
                    <td>
                    @if( $btn_ver == 1 )
