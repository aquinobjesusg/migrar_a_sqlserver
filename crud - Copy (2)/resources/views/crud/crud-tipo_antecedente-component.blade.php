...
@lang('dicc.Tipo_antecedente')
    ...
    <table id="example" class="display" style="width:100%">
        <thead>
<tr><th>@lang('dicc.NameOfId')</th><th>@lang('dicc.NameOfCodetipo')</th><th>@lang('dicc.NameOfDescripcion')</th><th>@lang('dicc.NameOfTipoantecedente')</th><th>@lang('dicc.Opciones')</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ($registros as $registro)
                <tr>
<td>{{ $registro->id }}</td><td>{{ $registro->codetipo }}</td><td>{{ $registro->descripcion }}</td><td>{{ $registro->tipoantecedente }}</td>
                    <td>
                    @if( $btn_ver == 1 )
